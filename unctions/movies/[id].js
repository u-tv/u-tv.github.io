// functions/movie/[id].js
const TMDB_API_KEY = '174d0214bf933dd59b3d5ec68a0c967f';
const IMG_BASE = 'https://image.tmdb.org/t/p/w500';
const SITE_URL = 'https://u-tv.pages.dev';   // ← यही आपका मुख्य डोमेन
...
export async function onRequest(context) {
  const { id } = context.params;   // movie ID from URL
...
<html lang="hi-IN">
<head><meta charset="UTF-8"><title>${escapeHtml(title)} (${year}) - Watch Free HD</title><meta name="description" content="${escapeHtml(overview.substring(0,160))}"><link rel="canonical" href="${SITE_URL}/movie/${id}"><meta property="og:title" content="${escapeHtml(title)}"><meta property="og:image" content="${backdrop}"><style>
*{margin:0;padding:0;box-sizing:border-box}body{background:#050508;color:#fff;font-family:sans-serif;padding:20px}.container{max-width:1000px;margin:0 auto}.backdrop{width:100%;border-radius:20px;margin-bottom:20px}.info{background:#1e1f2a;border-radius:20px;padding:20px}h1{color:#e50914}.meta{color:#aaa;margin:10px 0}.server-buttons{display:flex;gap:8px;flex-wrap:wrap;margin:20px 0}.server-btn{background:#e50914;border:none;padding:8px 14px;border-radius:40px;cursor:pointer;color:#fff}.server-btn:hover{background:#b00710}.player{position:relative;padding-bottom:56.25%;height:0;margin-top:20px}.player iframe{position:absolute;top:0;left:0;width:100%;height:100%;border:none;border-radius:16px}.footer{text-align:center;margin-top:30px;color:#666}</style></head>
<body><div class="container"><img class="backdrop" src="${backdrop}"><div class="info"><h1>${escapeHtml(title)} (${year})</h1>${tagline ? `<p>✨ ${escapeHtml(tagline)}</p>` : ''}<div class="meta">⭐ ${rating} (${voteCount}) | ${year} | ${runtime} | ${genres}</div><p>${escapeHtml(overview)}</p><div><strong>Cast:</strong> ${escapeHtml(cast)}</div><div><strong>Servers:</strong><div class="server-buttons">${serverButtons}</div></div></div><div class="player"><iframe id="player" src="${EMBED_SERVERS[0].url.replace('%ID%', id)}" allowfullscreen></iframe></div><div class="footer"><a href="/">← Home</a> | © U-TV</div></div>
<script>document.querySelectorAll('.server-btn').forEach(b=>b.onclick=()=>document.getElementById('player').src=b.dataset.url);</script></body></html>`;
