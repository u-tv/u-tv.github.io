// worker.js – Complete Movie Site (Homepage + Movie Pages + 10 Servers)
const TMDB_API_KEY = '174d0214bf933dd59b3d5ec68a0c967f';
const IMG_BASE = 'https://image.tmdb.org/t/p/w500';
const SITE_URL = 'https://u-tv.pages.dev';   // आपका डोमेन (बाद में बदल सकते हैं)
...
// Homepage – shows popular movies as cards
async function handleHome() {
  ...
  return `<!DOCTYPE html>
<html lang="hi-IN">
<head><meta charset="UTF-8"><title>U-TV – Watch Free Movies</title><meta name="description" content="Stream latest movies in HD. 10+ servers, no registration."><style>
...
// Movie detail page – full TMDB data + 10 embed servers
...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}body{background:#050508;color:#fff;font-family:sans-serif;padding:20px}.container{max-width:1000px;margin:0 auto}.backdrop{width:100%;border-radius:20px;margin-bottom:20px}.info{background:#1e1f2a;border-radius:20px;padding:20px;margin-bottom:20px}h1{color:#e50914;margin-bottom:10px}.meta{color:#aaa;margin:10px 0;display:flex;gap:20px;flex-wrap:wrap}.server-buttons{display:flex;gap:8px;flex-wrap:wrap;margin:20px 0}.server-btn{background:#e50914;border:none;padding:10px 16px;border-radius:40px;cursor:pointer;color:#fff;font-weight:bold;font-size:0.8rem}.server-btn:hover{background:#b00710}.player{position:relative;padding-bottom:56.25%;height:0;margin-top:20px}.player iframe{position:absolute;top:0;left:0;width:100%;height:100%;border:none;border-radius:16px}.footer{text-align:center;margin-top:30px;color:#666}@media (max-width:768px){.server-btn{padding:6px 12px;font-size:0.7rem}}
  </style>
</head>
<body>
<div class="container">
...
export default {
  async fetch(request) {
    ...
    if (match) {
      try {
        const html = await handleMovie(match[1]);
